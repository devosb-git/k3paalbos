...
create or replace function public.class_tasks_password_is_set() returns boolean language plpgsql security definer set search_path=public as $$
begin
 ...
 return exists(select 1 from public.class_task_settings where id=1 and reset_password_hash is not null);
end; $$;
...
create or replace function public.class_tasks_change_password(p_current_password text,p_new_password text) returns boolean language plpgsql security definer set search_path=public,extensions as $$
declare v_hash text;
begin
 if not exists(select 1 from public.profiles p where p.id=auth.uid() and p.active=true and p.role='teacher') then raise exception 'Geen toegang'; end if;
 ...
 if extensions.crypt(coalesce(p_current_password,''),v_hash)<>v_hash then raise exception 'Huidig beheerwachtwoord is verkeerd'; end if;
 if length(coalesce(p_new_password,''))<4 then raise exception 'Nieuw wachtwoord moet minstens 4 tekens bevatten'; end if;
 if extensions.crypt(coalesce(p_new_password,''),v_hash)=v_hash then raise exception 'Kies een ander nieuw wachtwoord'; end if;
 update public.class_task_settings set reset_password_hash=extensions.crypt(p_new_password,extensions.gen_salt('bf')),updated_at=now(),updated_by=auth.uid() where id=1;
 return true;
end; $$;
...
create or replace function public.class_tasks_reset(p_password text) returns boolean language plpgsql security definer set search_path=public,extensions as $$
declare v_hash text;
begin
 if not exists(select 1 from public.profiles p where p.id=auth.uid() and p.active=true and p.role='teacher') then raise exception 'Geen toegang'; end if;
 select reset_password_hash into v_hash from public.class_task_settings where id=1;
...
create or replace function public.class_tasks_new_school_year(p_password text) returns boolean language plpgsql security definer set search_path=public,extensions as $$
declare v_hash text;
begin
 if not exists(select 1 from public.profiles p where p.id=auth.uid() and p.active=true and p.role='teacher') then raise exception 'Geen toegang'; end if;
 select reset_password_hash into v_hash from public.class_task_settings where id=1;
