...
create table if not exists public.class_task_assignments (
  ...
  task_key text not null check (task_key in ('first_in_line','schoolbags','jackets','bottles','mail','wipe_table','sweep','empty_compost','water_plants','update_calendar')),
  slot smallint not null check (slot in (1,2)),
  student_id uuid not null references public.class_students(id) on delete cascade,
  task_cycle integer not null default 1, created_at timestamptz not null default now(), created_by uuid references auth.users(id),
  unique (week_start, task_key, slot)
);

create table if not exists public.class_week_sunshine (
  week_start date primary key, student_id uuid not null references public.class_students(id) on delete cascade,
  sunshine_cycle integer not null default 1, created_at timestamptz not null default now(), created_by uuid references auth.users(id)
);

create table if not exists public.class_task_settings (
  id smallint primary key default 1 check (id = 1), reset_password_hash text,
...
create policy "active users read class students" on public.class_students for select to authenticated using (exists(select 1 from public.profiles p where p.id=auth.uid() and p.active=true));
drop policy if exists "active users read class task assignments" on public.class_task_assignments;
create policy "active users read class task assignments" on public.class_task_assignments for select to authenticated using (exists(select 1 from public.profiles p where p.id=auth.uid() and p.active=true));
drop policy if exists "active users read class sunshine" on public.class_week_sunshine;
create policy "active users read class sunshine" on public.class_week_sunshine for select to authenticated using (exists(select 1 from public.profiles p where p.id=auth.uid() and p.active=true));

drop policy if exists "teachers manage class students" on public.class_students;
create policy "teachers manage class students" on public.class_students for all to authenticated using (exists(select 1 from public.profiles p where p.id=auth.uid() and p.active=true and p.role='teacher')) with check (exists(select 1 from public.profiles p where p.id=auth.uid() and p.active=true and p.role='teacher'));
drop policy if exists "teachers manage class task assignments" on public.class_task_assignments;
create policy "teachers manage class task assignments" on public.class_task_assignments for all to authenticated using (exists(select 1 from public.profiles p where p.id=auth.uid() and p.active=true and p.role='teacher')) with check (exists(select 1 from public.profiles p where p.id=auth.uid() and p.active=true and p.role='teacher'));
drop policy if exists "teachers manage class sunshine" on public.class_week_sunshine;
create policy "teachers manage class sunshine" on public.class_week_sunshine for all to authenticated using (exists(select 1 from public.profiles p where p.id=auth.uid() and p.active=true and p.role='teacher')) with check (exists(select 1 from public.profiles p where p.id=auth.uid() and p.active=true and p.role='teacher'));
...
begin
  ...
  update public.class_task_settings set reset_password_hash=extensions.crypt(p_new_password, extensions.gen_salt('bf')), updated_at=now(), updated_by=auth.uid() where id=1;
  return true;
end; $$;
...
begin
  ...
  if v_hash is null then raise exception 'Resetwachtwoord is nog niet ingesteld'; end if;
  if extensions.crypt(coalesce(p_password,''), v_hash) <> v_hash then raise exception 'Verkeerd resetwachtwoord'; end if;
  -- Expliciete WHERE voorkomt Supabase/Postgres delete-without-filter beveiliging.
  delete from public.class_task_assignments where id is not null;
  delete from public.class_week_sunshine where week_start is not null;
  return true;
end; $$;
...
begin
  ...
  delete from public.class_task_assignments where id is not null;
  delete from public.class_week_sunshine where week_start is not null;
  delete from public.class_students where id is not null;
  return true;
end; $$;
