...
const clothingAssets=import.meta.glob('./clothing-builder/*.png',{
  eager:true,
  query:'?url',
  import:'default'
});
...
function assetUrl(filename){
  return clothingAssets[`./clothing-builder/${filename}`]||'';
}
...
function modelImage(gender){
  const key=modelKey();
  const filename=`kleding-${gender}-${key}.png`;
  const fallback=`kleding-${gender}-neutraal.png`;
  const src=assetUrl(filename)||assetUrl(fallback);
  const label=gender==='meisje'?'Meisje':'Jongen';
  return `<div class="clothing-model-frame"><img class="clothing-model-image" src="${src}" alt="${label}" draggable="false"></div>`;
}

function buttonImage(id,label){
  const src=assetUrl(buttonFiles[id]);
  return `<img class="clothing-choice-image" src="${src}" alt="${label}" draggable="false">`;
}
...
function styles(){
  ...
  style.textContent=`
    ...
    .clothing-model-frame{width:180px;height:245px;display:grid;place-items:end center;overflow:hidden;border-radius:20px;background:#fff}
    .clothing-model-image{display:block;max-width:100%;max-height:100%;width:auto;height:auto;object-fit:contain;user-select:none;pointer-events:none}
    ...
    .clothing-choice-image{display:block;width:100%;height:100%;object-fit:contain;user-select:none;pointer-events:none}
    ...
    @media(max-width:560px){.clothing-builder-card{padding:12px}.clothing-children{min-height:205px}.clothing-model-frame{width:142px;height:198px}.clothing-choice{width:144px;height:69px}.clothing-builder-intro h2{font-size:23px}}
  `;
