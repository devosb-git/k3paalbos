...
async function eraseWeek(){
  if(!confirm('Wil je alle figuren en kleding van maandag tot en met vrijdag wissen?'))return;
  try{
...
export async function showClothing(navigate,profile){
  ...
  document.querySelector('#app').innerHTML=`<main class="page clothing-page"><header class="topbar"><div class="brand"><div class="fox">🦊</div><div><h1>De Vosjes</h1><p>Wat trek ik aan?</p></div></div><nav class="main-nav"><button class="nav-item" data-page="calendar"><span>📅</span><small>Kalender</small></button><button class="nav-item" data-page="week"><span>🗓️</span><small>Weekkalender</small></button><button class="nav-item active"><span>👕</span><small>Kleding</small></button><button class="nav-item" data-page="weather"><span>🌤️</span><small>Weer</small></button><button class="nav-item" data-page="day"><span>➡️</span><small>Dagkalender</small></button></nav><div class="account">${name} <button id="clothing-logout">Uitloggen</button></div></header><section class="clothing-builder-card"><div class="clothing-builder-intro"><div><h2>Kleed de kinderen aan</h2><p>Kies voor elke schooldag een figuur en kleed die aan. Je keuzes worden automatisch bewaard.</p></div><button type="button" class="clothing-week-clear" id="clothing-week-clear">Week wissen</button></div><div class="clothing-week-grid"></div><div class="clothing-editor-head"></div><div class="clothing-builder-rows"></div><div class="clothing-status" id="clothing-status"></div></section>${genderPopup()}</main>`;
