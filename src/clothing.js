...
function weekCards(){
  return weekDates.map((date,index)=>{
    ...
    const body=state?.gender
      ? `<img class="clothing-day-figure ${state.gender==='meisje'?'clothing-day-figure-girl':''}" src="${modelSrc(state.gender,state)}" alt="${state.gender==='meisje'?'Meisje':'Jongen'}"><button type="button" class="clothing-day-clear" data-clear-day="${date}" aria-label="${dayNames[index]} opnieuw beginnen">×</button>`
...
async function chooseGender(gender){
  ...
  try{await saveDay(date);}catch(error){console.error(error);}
}
...
async function applyChoice(row,value){
  ...
  try{await saveDay(activeDate);}catch(error){console.error(error);}
}
async function clearDay(date){
  if(!date||!weekStates[date])return;
  try{
    const {error}=await supabase.from('clothing_week_days').delete().eq('clothing_date',date);
    if(error)throw error;
    delete weekStates[date];
...
async function eraseWeek(){
  ...
  try{
    ...
    if(error)throw error;
    weekStates={};activeDate='';refreshBuilder();
  }catch(error){console.error(error);}
}
...
export async function showClothing(navigate,profile){
  ...
  try{await loadWeek();refreshBuilder();}catch(error){console.error(error);refreshBuilder();}
}