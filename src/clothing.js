...
function getWeekDates(offset=weekOffset){const now=new Date();const monday=new Date(now.getFullYear(),now.getMonth(),now.getDate());monday.setDate(monday.getDate()-((monday.getDay()+6)%7)+(offset*7));return Array.from({length:5},(_,i)=>{const d=new Date(monday);d.setDate(monday.getDate()+i);return formatDateLocal(d);});}
...
function isHistoryWeek(){return weekOffset<0;}
function weekLabel(){return isHistoryWeek()?`Vorige week · ${shortDate(weekDates[0])} – ${shortDate(weekDates[4])}`:`Deze week · ${shortDate(weekDates[0])} – ${shortDate(weekDates[4])}`;}
...
async function loadWeek(){
  ...
  weekStates={};activeDate='';
  (data||[]).forEach(row=>{weekStates[row.clothing_date]=normalizeDay(row);});
  if(!isHistoryWeek())activeDate=weekDates.find(date=>weekStates[date])||'';
}
async function saveDay(date){
  const state=weekStates[date];
  if(!state?.gender||isHistoryWeek())return;
...
function weekCards(){
  const history=isHistoryWeek();
  return weekDates.map((date,index)=>{
    const state=weekStates[date];
    const active=date===activeDate;
    const body=state?.gender
      ? `<img class="clothing-day-figure ${state.gender==='meisje'?'clothing-day-figure-girl':''}" src="${modelSrc(state.gender,state)}" alt="${state.gender==='meisje'?'Meisje':'Jongen'}">${history?'':`<button type="button" class="clothing-day-clear" data-clear-day="${date}" aria-label="${dayNames[index]} opnieuw beginnen">×</button>`}`
      : history?`<div class="clothing-history-empty">—</div>`:`<button class="clothing-day-plus" type="button" data-add-day="${date}" aria-label="Figuur kiezen voor ${dayNames[index]}">+</button>`;
    return `<article class="clothing-day-card ${active?'active':''} ${history?'history':''}" data-day-card="${date}"><div class="clothing-day-heading"><strong>${dayNames[index]}</strong><small>${shortDate(date)}</small></div><div class="clothing-day-body">${body}</div></article>`;
  }).join('');
}
function editorIntro(){
  if(isHistoryWeek())return `<div class="clothing-editor-empty"><strong>Terugkijken</strong><span>Dit is een bewaarde week. Ga terug naar deze week om kleding te kiezen of te wijzigen.</span></div>`;
  ...
  const i=weekDates.indexOf(activeDate);const state=currentState();
...
function rowEnabled(row,state){
  if(isHistoryWeek()||!state?.gender)return false;
...
function rowMessage(row,state){
  if(isHistoryWeek())return 'Vorige weken zijn alleen om terug te kijken.';
...
function genderPopup(){return `<div class="clothing-popup-backdrop" id="clothing-gender-popup" hidden><div class="clothing-popup" role="dialog" aria-modal="true" aria-labelledby="clothing-popup-title"><button class="clothing-popup-close" id="clothing-popup-close" type="button" aria-label="Sluiten">×</button><h3 id="clothing-popup-title">Kies een figuur</h3><p>Kies wie je voor deze dag wilt aankleden.</p><div class="clothing-gender-options"><button type="button" class="clothing-gender-choice" data-gender="jongen"><img src="${assetUrl('kleding-jongen-neutraal.png')}" alt="Jongen"><span>JONGEN</span></button><button type="button" class="clothing-gender-choice" data-gender="meisje"><img src="${assetUrl('kleding-meisje-neutraal.png')}" alt="Meisje"><span>MEISJE</span></button></div></div></div>`;}
function eraseConfirmPopup(){return `<div class="clothing-confirm-overlay" id="clothing-confirm-overlay" hidden><div class="clothing-confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="clothing-confirm-title"><div class="clothing-confirm-icon">🧹</div><h2 id="clothing-confirm-title">Week wissen?</h2><p>Wil je alle figuren en kleding van maandag tot en met vrijdag wissen? Deze actie kan niet ongedaan worden gemaakt.</p><div class="clothing-confirm-actions"><button type="button" class="clothing-confirm-cancel" id="clothing-confirm-cancel">Annuleren</button><button type="button" class="clothing-confirm-ok" id="clothing-confirm-ok">Alles wissen</button></div></div></div>`;}
function openGenderPopup(date){if(isHistoryWeek())return;pendingGenderDate=date;const popup=document.getElementById('clothing-gender-popup');if(popup)popup.hidden=false;}
...
function askEraseConfirmation(){return new Promise(resolve=>{eraseConfirmResolver=resolve;const overlay=document.getElementById('clothing-confirm-overlay');if(overlay){overlay.hidden=false;document.body.classList.add('clothing-confirm-open');}});}
function closeEraseConfirmation(result){const overlay=document.getElementById('clothing-confirm-overlay');if(overlay)overlay.hidden=true;document.body.classList.remove('clothing-confirm-open');const resolve=eraseConfirmResolver;eraseConfirmResolver=null;if(resolve)resolve(result);}
...
function bindInteractions(){
  ...
  document.querySelectorAll('[data-clear-day]').forEach(button=>button.onclick=event=>{event.preventDefault();event.stopPropagation();clearDay(button.dataset.clearDay);});
  document.querySelectorAll('[data-day-card]').forEach(card=>card.onclick=()=>{const date=card.dataset.dayCard;if(!isHistoryWeek()&&weekStates[date]){activeDate=date;refreshBuilder();}});
...
function refreshBuilder(){
  const label=document.getElementById('clothing-week-label');if(label)label.textContent=weekLabel();
  ...
  const clear=document.getElementById('clothing-week-clear');if(clear)clear.hidden=isHistoryWeek();
  const current=document.getElementById('clothing-current-week');if(current)current.hidden=!isHistoryWeek();
  bindInteractions();
}
async function changeWeek(offset){weekOffset=offset;await loadWeek();refreshBuilder();}
async function chooseGender(gender){const date=pendingGenderDate;if(!date||isHistoryWeek())return;weekStates[date]=emptyDay(gender);activeDate=date;closeGenderPopup();refreshBuilder();try{await saveDay(date);}catch(error){console.error(error);}}
function toggleTop(state,value){let tops=[...state.tops];if(tops.includes(value))tops=tops.filter(item=>item!==value);else{if(value==='regenjas')tops=tops.filter(item=>item!=='winterjas');if(value==='winterjas')tops=tops.filter(item=>item!=='regenjas');tops.push(value);}return {...state,tops,shoes:'',accessory:''};}
async function applyChoice(row,value){if(!activeDate||isHistoryWeek())return;const state=currentState();if(!optionAllowed(row,value,state))return;let next={...state,tops:[...state.tops]};if(row==='pants')next={...emptyDay(state.gender),pants:value};if(row==='top')next=toggleTop(state,value);if(row==='shoes')next={...state,tops:[...state.tops],shoes:value,accessory:''};if(row==='accessory')next={...state,tops:[...state.tops],accessory:state.accessory===value?'':value};weekStates[activeDate]=next;refreshBuilder();try{await saveDay(activeDate);}catch(error){console.error(error);}}
async function clearDay(date){if(!date||!weekStates[date]||isHistoryWeek())return;try{const {error}=await supabase.from('clothing_week_days').delete().eq('clothing_date',date);if(error)throw error;delete weekStates[date];if(activeDate===date)activeDate='';refreshBuilder();}catch(error){console.error(error);}}
async function eraseWeek(){if(isHistoryWeek()||!(await askEraseConfirmation()))return;try{const {error}=await supabase.from('clothing_week_days').delete().gte('clothing_date',weekDates[0]).lte('clothing_date',weekDates[4]);if(error)throw error;weekStates={};activeDate='';refreshBuilder();}catch(error){console.error(error);}}
...
.clothing-page{min-height:100vh}.clothing-builder-card{max-width:1500px;margin:0 auto;background:#fff;border:2px solid #dce9db;border-radius:24px;box-shadow:0 8px 24px #234c2712;padding:18px 22px 22px}.clothing-builder-intro{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;text-align:left;margin:0 auto 8px}.clothing-builder-intro h2{font-size:28px;color:#285d39;margin:0}.clothing-builder-intro p{color:#718176;font-size:14px;margin:4px 0 0}.clothing-week-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.clothing-week-clear{border:2px solid #e6aaaa;background:#fff;color:#b44f4f;border-radius:13px;padding:9px 13px;font-weight:bold;cursor:pointer}.clothing-week-nav{border:2px solid #d5e4d2;background:#fff;color:#356342;border-radius:13px;padding:9px 13px;font-weight:800;cursor:pointer}.clothing-week-nav:hover{background:#f2f8ef}.clothing-week-bar{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:6px 0 4px}.clothing-week-label{font-size:14px;font-weight:800;color:#58705d}.clothing-week-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:12px;margin:10px 0 18px}.clothing-day-card{min-height:245px;border:2px solid #e3ebe0;border-radius:18px;background:#fdf3de;overflow:hidden;cursor:pointer;transition:.14s;display:flex;flex-direction:column}.clothing-day-card.history{cursor:default}.clothing-day-card:hover{transform:translateY(-2px);border-color:#bfd7ba}.clothing-day-card.history:hover{transform:none}.clothing-day-card.active{border-color:#4f9a61;box-shadow:0 0 0 3px #d7ecd5}.clothing-day-heading{display:flex;align-items:baseline;justify-content:space-between;padding:10px 12px 5px;color:#31593b}.clothing-day-heading strong{font-size:17px}.clothing-day-heading small{color:#7f8f80}.clothing-day-body{position:relative;flex:1;min-height:195px;display:grid;place-items:end center;padding:3px 8px 8px}.clothing-history-empty{margin:auto;color:#b5b8ad;font-size:34px}.clothing-day-plus{width:74px;height:74px;align-self:center;border:2px dashed #b9cdb5;border-radius:50%;background:#fffaf0;color:#6f9271;font-size:48px;font-weight:300;line-height:1;cursor:pointer;margin:auto}.clothing-day-figure{display:block;width:100%;height:195px;object-fit:contain;object-position:center bottom;user-select:none;pointer-events:none}.clothing-day-figure-girl{transform:translateY(-5px)}.clothing-day-clear{position:absolute;right:8px;bottom:8px;width:30px;height:30px;border:2px solid #e6aaaa;border-radius:50%;background:#fff;color:#b44f4f;font-size:20px;font-weight:800;line-height:1;display:grid;place-items:center;padding:0;box-shadow:0 2px 7px #6e3b3b16;z-index:2}.clothing-day-clear:hover{background:#fff5f5;border-color:#d98c8c;transform:scale(1.05)}.clothing-editor-head{margin:4px 8px 8px}.clothing-editor-title,.clothing-editor-empty{display:flex;align-items:baseline;gap:10px;color:#31593b}.clothing-editor-title strong,.clothing-editor-empty strong{font-size:18px}.clothing-editor-title span,.clothing-editor-empty span{font-size:13px;color:#718176}.clothing-builder-row{display:grid;grid-template-columns:220px 1fr;gap:16px;align-items:center;padding:12px 8px;border-top:2px solid #edf2eb}.clothing-row-copy h3{margin:0;color:#31593b;font-size:20px}.clothing-row-copy p{margin:3px 0 0;color:#718176;font-size:13px;line-height:1.25}.clothing-builder-row.locked .clothing-row-copy{opacity:.65}.clothing-row-options{display:flex;justify-content:flex-start;gap:10px;flex-wrap:wrap}.clothing-choice{position:relative;width:180px;height:86px;border:2px solid #e2e9df;border-radius:15px;background:#fdf3de;padding:0;overflow:hidden;box-shadow:0 2px 5px #254d2610;transition:.12s}.clothing-choice:not(:disabled):hover{transform:translateY(-2px);border-color:#b8d7b6}.clothing-choice.selected{border-color:#4f9a61;box-shadow:0 0 0 3px #d7ecd5}.clothing-choice:disabled{opacity:.28;filter:grayscale(.35);cursor:not-allowed}.clothing-choice-image{display:block;width:100%;height:100%;object-fit:contain;user-select:none;pointer-events:none}.clothing-choice-check{display:none;position:absolute;right:6px;top:6px;width:25px;height:25px;border-radius:50%;background:#4f9a61;color:#fff;font-size:17px;font-weight:900;align-items:center;justify-content:center}.clothing-choice.selected .clothing-choice-check{display:flex}.clothing-popup-backdrop{position:fixed;inset:0;background:#24362b66;display:grid;place-items:center;z-index:1000;padding:18px}.clothing-popup-backdrop[hidden]{display:none}.clothing-popup{position:relative;width:min(620px,94vw);background:#fff;border-radius:24px;border:2px solid #dce9db;box-shadow:0 22px 70px #14271933;padding:24px}.clothing-popup h3{margin:0;color:#31593b;font-size:26px}.clothing-popup p{margin:4px 0 18px;color:#718176}.clothing-popup-close{position:absolute;right:14px;top:12px;width:38px;height:38px;border:0;border-radius:50%;background:#f3eee6;color:#6c6257;font-size:25px;cursor:pointer}.clothing-gender-options{display:grid;grid-template-columns:1fr 1fr;gap:16px}.clothing-gender-choice{border:2px solid #e2e9df;border-radius:20px;background:#fdf3de;padding:12px;cursor:pointer;color:#31593b;font-weight:900;font-size:17px}.clothing-gender-choice:hover{border-color:#77ad78;transform:translateY(-2px)}.clothing-gender-choice img{display:block;width:100%;height:250px;object-fit:contain;margin:0 auto 8px}.clothing-page .main-nav .nav-item.active{background:#e8f4e7;border-color:#cce2ca;color:#23623a}
...
export async function showClothing(navigate,profile){
  ...
  document.querySelector('#app').innerHTML=`<main class="page clothing-page"><header class="topbar"><div class="brand"><div class="fox">🦊</div><div><h1>De Vosjes</h1><p>Wat trek ik aan?</p></div></div><nav class="main-nav"><button class="nav-item" data-page="calendar"><span>📅</span><small>Kalender</small></button><button class="nav-item" data-page="week"><span>🗓️</span><small>Weekkalender</small></button><button class="nav-item active"><span>👕</span><small>Kleding</small></button><button class="nav-item" data-page="weather"><span>🌤️</span><small>Weer</small></button><button class="nav-item" data-page="day"><span>➡️</span><small>Dagkalender</small></button></nav><div class="account">${name} <button id="clothing-logout">Uitloggen</button></div></header><section class="clothing-builder-card"><div class="clothing-builder-intro"><div><h2>Kleed de kinderen aan</h2><p>Kies voor elke schooldag een figuur en kleed die aan. Je keuzes worden automatisch bewaard.</p></div><div class="clothing-week-actions"><button type="button" class="clothing-week-nav" id="clothing-previous-week">← Vorige week</button><button type="button" class="clothing-week-nav" id="clothing-current-week" hidden>Deze week</button><button type="button" class="clothing-week-clear" id="clothing-week-clear">🗑️ Wissen</button></div></div><div class="clothing-week-bar"><span class="clothing-week-label" id="clothing-week-label"></span></div><div class="clothing-week-grid"></div><div class="clothing-editor-head"></div><div class="clothing-builder-rows"></div></section>${genderPopup()}${eraseConfirmPopup()}</main>`;
  document.querySelectorAll('[data-page]').forEach(button=>button.addEventListener('click',()=>navigate(button.dataset.page)));
  document.getElementById('clothing-previous-week').onclick=()=>changeWeek(weekOffset-1);
  document.getElementById('clothing-current-week').onclick=()=>changeWeek(0);
  ...
  try{weekOffset=0;await loadWeek();refreshBuilder();}catch(error){console.error(error);refreshBuilder();}
}