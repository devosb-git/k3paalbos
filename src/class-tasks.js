...
const tasks=[
  ['first_in_line','🚶','Eerste in de rij'],['schoolbags','🎒','Boekentassen'],['jackets','🧥','Jassen'],['bottles','🍼','Flessen'],['mail','✉️','Briefwisseling'],['wipe_table','🧽','Tafel poetsen'],['sweep','🧹','Vegen'],['empty_compost','🌱','Compost legen'],['water_plants','🪴','Planten water geven'],['update_calendar','📅','Kalender aanvullen']
...
async function loadData(){
  ...
  if(profile?.role==='teacher'&&!results[3]?.error)passwordIsSet=!!results[3].data;
}
...
function render(){
  ...
  const assignmentMap=new Map();currentAssignments.forEach(a=>{if(!assignmentMap.has(a.task_key))assignmentMap.set(a.task_key,[]);assignmentMap.get(a.task_key).push(a)});
  const taskCards=tasks.map(task=>{const assigned=(assignmentMap.get(task.key)||[]).sort((a,b)=>a.slot-b.slot);return `<article class="task-card"><div class="task-icon">${task.icon}</div><div><h3>${task.label}</h3><div class="task-students">${[0,1].map(i=>assigned[i]?`<span class="student-chip">${studentName(assigned[i].student_id)}</span>`:'<span class="student-chip empty">Nog te verdelen</span>').join('')}</div></div></article>`}).join('');
  const roster=students.map(s=>s.name).join('\n');
  const passwordPanel=!passwordIsSet?`<div class="admin-row"><input id="reset-new-password" type="password" placeholder="Beheerwachtwoord"><button class="admin-button" id="set-reset-password">Wachtwoord instellen</button></div>`:`<div><p><strong>Beheerwachtwoord is ingesteld.</strong> Om het te wijzigen heb je het huidige wachtwoord nodig.</p><div class="admin-row"><input id="change-current-password" type="password" placeholder="Huidig wachtwoord"><input id="change-new-password" type="password" placeholder="Nieuw wachtwoord"><button class="admin-button" id="change-password">Wachtwoord wijzigen</button></div></div>`;
  app().innerHTML=`<main class="page tasks-page">${header()}<section class="tasks-board"><div class="tasks-title"><div><h2>🎲 Klastaken</h2><p>De dobbelsteen verdeelt elke taak eerlijk over twee kinderen.</p></div>${canEdit?'<button class="dice-button" id="roll-tasks"><span>🎲</span> Dobbelen!</button>':''}</div><section class="sunshine-card"><div class="sunshine-icon">☀️</div><div><h3>Zonnetje van de week</h3><div class="sunshine-name">${currentSunshine?studentName(currentSunshine.student_id):'Nog te kiezen'}</div></div></section><div class="tasks-grid">${taskCards}</div>${canEdit?`<details class="tasks-admin"><summary>⚙️ Beheer klastaken</summary><div class="tasks-admin-grid"><section class="admin-card"><h3>Leerlingen</h3><p>Eén naam per regel. Namen die je verwijdert worden inactief maar hun geschiedenis blijft bewaard.</p><textarea class="student-editor" id="student-editor" placeholder="Anna\nBram\n...">${roster}</textarea><div class="admin-row"><input id="students-password" type="password" placeholder="Beheerwachtwoord"><button class="admin-button" id="save-students">Leerlingen opslaan</button></div></section><section class="admin-card"><h3>Beveiliging</h3>${passwordPanel}<p style="margin-top:14px">De gewone reset wist alleen de taakgeschiedenis. Nieuw schooljaar wist ook de volledige leerlingenlijst.</p><div class="admin-row" style="margin-top:12px"><input id="reset-password" type="password" placeholder="Beheerwachtwoord"><button class="danger-button" id="reset-tasks">Taakgeschiedenis resetten</button></div><div class="admin-row" style="margin-top:12px"><input id="school-year-password" type="password" placeholder="Beheerwachtwoord"><button class="danger-button" id="new-school-year">🎓 Nieuw schooljaar</button></div><p style="margin-top:10px"><strong>Nieuw schooljaar</strong> wist alle leerlingen, taakverdelingen, taakgeschiedenis en zonnetjes. Deze actie kan niet ongedaan worden gemaakt.</p></section></div></details>`:''}${statusMessage?`<div class="tasks-status ${statusError?'tasks-error':''}">${statusMessage}</div>`:''}</section></main>`;
  bind(canEdit);
}
...
function bind(canEdit){
  ...
  document.querySelector('#set-reset-password')?.addEventListener('click',setResetPassword);
...
async function rollTasks(){
  ...
  const button=document.querySelector('#roll-tasks');button.disabled=true;button.classList.add('rolling');statusMessage='De dobbelsteen rolt…';statusError=false;render();await new Promise(r=>setTimeout(r,850));
  const week=mondayKey();
  ...
  const {error:sunError}=await supabase.from('class_week_sunshine').insert({week_start:week,student_id:sunshine.id,sunshine_cycle:sunCycle,created_by:profile.id});if(sunError)return showStatus(sunError.message,true);
...
async function setResetPassword(){
  ...
  if(value.length<4)return showStatus('Het beheerwachtwoord moet minstens 4 tekens lang zijn.',true);
  const {error}=await supabase.rpc('class_tasks_set_reset_password',{p_new_password:value});if(error)return showStatus(error.message,true);
  await loadData();showStatus('Beheerwachtwoord ingesteld.');
}

async function changePassword(){
  const current=document.querySelector('#change-current-password').value;
  const next=document.querySelector('#change-new-password').value;
  if(!current)return showStatus('Vul het huidige beheerwachtwoord in.',true);
  if(next.length<4)return showStatus('Het nieuwe wachtwoord moet minstens 4 tekens lang zijn.',true);
  const {error}=await supabase.rpc('class_tasks_change_password',{p_current_password:current,p_new_password:next});
  if(error)return showStatus(error.message,true);
  await loadData();showStatus('Beheerwachtwoord gewijzigd. Gebruik voortaan het nieuwe wachtwoord.');
}
...
async function openTasks(){captureNavigation();profile=await getProfile();if(!profile)return;pageActive=true;statusMessage='';statusError=false;await loadData();render();}
