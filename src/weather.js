...
.weather-header h2{color:#285d39;font-size:clamp(28px,4vw,40px);margin:0}.weather-header p{color:#718176;margin:5px 0 0}
.weather-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}
.weather-option{border:3px solid #e1eadf;background:#fbfdf9;border-radius:20px;min-height:165px;padding:14px 8px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;transition:.15s;touch-action:manipulation;cursor:pointer}
.weather-option:hover{background:#f1f8ef;transform:translateY(-2px)}.weather-option.selected{border-color:#4f9a61;background:#e8f4e7;box-shadow:0 0 0 4px #d7ecd5}
.weather-option img{width:105px;height:105px;object-fit:contain}.weather-option small{font-size:15px;font-weight:bold;color:#496153;text-align:center}
...
@media(max-width:700px){.weather-page{padding:9px}.weather-card{padding:16px;border-radius:22px}.weather-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.weather-option{min-height:145px}.weather-option img{width:82px;height:82px}.weather-option small{font-size:12px}}
@media(max-width:430px){.weather-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.weather-option{min-height:145px}}
`;
  document.head.appendChild(s);
}

function enableWeatherNav(){
  const weather=[...document.querySelectorAll('.nav-item')].find(b=>b.textContent.includes('Weer'));
  if(!weather)return;
  weather.disabled=false;weather.dataset.weatherNav='true';
  if(weather.dataset.bound)return;
  weather.dataset.bound='true';weather.addEventListener('click',e=>{e.preventDefault();history.replaceState(null,'','#weather');renderWeather()});
}

function navigate(page){history.replaceState(null,'',page==='week'?'#week':'');location.reload()}

function renderWeather(){
  addStyles();document.title='Weer | De Vosjes';
  ...
  document.querySelectorAll('.weather-option').forEach(b=>b.onclick=()=>saveWeather(b.dataset.weather));
...
addStyles();
observer=new MutationObserver(()=>enableWeatherNav());observer.observe(document.body,{childList:true,subtree:true});
setTimeout(async()=>{enableWeatherNav();await loadWeather();if(location.hash==='#weather')renderWeather()},0);
