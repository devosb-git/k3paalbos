...
function addStyles(){
  ...
  style.textContent=`
    ...
    .week-picker-relative{margin:7px 7px 0;min-height:48px;border:2px dashed #c7d7c4;border-radius:11px;background:#fff;color:#607768;font-weight:800;font-size:14px;padding:5px;display:flex;align-items:center;justify-content:center;text-align:center;width:calc(100% - 14px)}
...
function closePicker(){openPopover?.remove();openPopover=null;}
function positionPicker(pop,anchor){const margin=8,gap=6,rect=anchor.getBoundingClientRect();const width=pop.offsetWidth,height=pop.offsetHeight;let left=rect.left;if(left+width>window.innerWidth-margin)left=rect.right-width;left=Math.max(margin,Math.min(left,window.innerWidth-width-margin));let top=rect.bottom+gap;if(top+height>window.innerHeight-margin)top=rect.top-height-gap;top=Math.max(margin,Math.min(top,window.innerHeight-height-margin));pop.style.left=`${Math.round(left)}px`;pop.style.top=`${Math.round(top)}px`;}
function openBasePicker(anchor,title){closePicker();const pop=document.createElement('div');pop.className='week-picker-popover';pop.innerHTML=`<div class="week-picker-popover-head"><strong>${title}</strong><button type="button" class="week-picker-close" aria-label="Sluiten">×</button></div><div class="week-picker-body"></div>`;document.body.appendChild(pop);pop.querySelector('.week-picker-close').onclick=closePicker;openPopover=pop;requestAnimationFrame(()=>positionPicker(pop,anchor));return {pop,body:pop.querySelector('.week-picker-body')};}
function refreshWeek(){closePicker();window.dispatchEvent(new CustomEvent('k3paalbos:navigate',{detail:{page:'week'}}));}
function setUniquePart(slot,kind,value){updateWeekState(state=>{const bucket=kind==='number'?'dayNumbers':'dayNames';const id=value.id;Object.keys(state[bucket]||{}).forEach(key=>{if(state[bucket][key]?.id===id)delete state[bucket][key]});state[bucket]=state[bucket]||{};state[bucket][slot]=value;});refreshWeek();}
function openDayPartPicker(anchor,slot,kind){const number=kind==='number';const {body}=openBasePicker(anchor,number?'Kies een cijfer':'Kies een dag');const options=number?weekNumbers:weekDayNames;body.className=`week-picker-body week-picker-simple-grid ${number?'':'days'}`;body.innerHTML=options.map(item=>`<button type="button" class="week-picker-simple" data-id="${item.id}">${number?item.label:item.name.toLowerCase()}</button>`).join('');body.querySelectorAll('.week-picker-simple').forEach(button=>button.onclick=()=>setUniquePart(slot,kind,options.find(item=>item.id===+button.dataset.id)));}
function setRelative(slot,term){updateWeekState(state=>{state.relativeDays=state.relativeDays||{};Object.keys(state.relativeDays).forEach(key=>{if(state.relativeDays[key]===term.id)delete state.relativeDays[key]});state.relativeDays[slot]=term.id;});refreshWeek();}
function openRelativePicker(anchor,slot){const {body}=openBasePicker(anchor,'Kies een begrip');body.className='week-picker-body week-picker-simple-grid';body.innerHTML=relativeTerms.map(term=>`<button type="button" class="week-picker-simple" data-id="${term.id}">${term.arrow} ${term.label}</button>`).join('');body.querySelectorAll('.week-picker-simple').forEach(button=>button.onclick=()=>setRelative(slot,relativeTerms.find(term=>term.id===button.dataset.id)));}
function saveActivity(slot,index,item){updateWeekState(state=>{state.activities=state.activities||{};const list=[...(state.activities[slot]||[])];if(index<list.length)list[index]={icon:item[0],label:item[1]};else list.push({icon:item[0],label:item[1]});state.activities[slot]=list;});refreshWeek();}
function removeActivity(slot,index){updateWeekState(state=>{state.activities=state.activities||{};state.activities[slot]=(state.activities[slot]||[]).filter((_,i)=>i!==index);});refreshWeek();}
function renderActivityOptions(body,slot,index,groupIndex){const group=groups[groupIndex];body.innerHTML=`<div class="week-picker-tabs">${groups.map((g,i)=>`<button type="button" class="week-picker-tab ${i===groupIndex?'active':''}" data-group="${i}">${g.name}</button>`).join('')}</div><div class="week-picker-options">${group.items.map((item,i)=>`<button type="button" class="week-picker-option" data-item="${i}"><span class="option-icon">${activityIconMarkup(item[0],item[1])}</span><small>${item[1]}</small></button>`).join('')}</div>`;body.querySelectorAll('.week-picker-tab').forEach(button=>button.onclick=()=>renderActivityOptions(body,slot,index,+button.dataset.group));body.querySelectorAll('.week-picker-option').forEach(button=>button.onclick=()=>saveActivity(slot,index,group.items[+button.dataset.item]));requestAnimationFrame(()=>{if(openPopover)positionPicker(openPopover,document.querySelector(`.week-picker-slot[data-slot="${slot}"][data-index="${index}"]`)||document.body)});}
function openActivityPicker(anchor,slot,index){const {body}=openBasePicker(anchor,index<(getWeekState().activities?.[slot]||[]).length?'Vervang activiteit':'Kies activiteit');renderActivityOptions(body,slot,index,0);requestAnimationFrame(()=>positionPicker(openPopover,anchor));}
function mount(){if(mounting)return;const wrap=document.querySelector('.week-wrap');const content=document.querySelector('.week-content');const columns=[...document.querySelectorAll('.week-column[data-slot]')];if(!wrap||!content||columns.length!==7||wrap.dataset.compactPicker==='1')return;const canEdit=!!document.querySelector('#clear-week');if(!canEdit)return;mounting=true;addStyles();wrap.dataset.compactPicker='1';content.classList.add('compact-picker-layout');const intro=wrap.querySelector('.week-title p');if(intro)intro.textContent='Tik op een vakje om een dag, begrip of activiteit te kiezen.';wrap.querySelector('.day-palette-bottom')?.remove();const state=getWeekState();columns.forEach(column=>{const slot=+column.dataset.slot;const number=state.dayNumbers?.[slot];const name=state.dayNames?.[slot];const head=column.querySelector('.week-day-head');const numberDrop=head?.querySelector('.week-head-drop.number');const nameDrop=head?.querySelector('.week-head-drop.name');if(numberDrop){numberDrop.innerHTML=`<button type="button" class="week-picker-head-button number ${number?'':'empty'}">${number?.label||'Cijfer +'}</button>`;numberDrop.onclick=()=>openDayPartPicker(numberDrop,slot,'number');}if(nameDrop){nameDrop.innerHTML=`<button type="button" class="week-picker-head-button ${name?'':'empty'}">${name?.name?String(name.name).toLowerCase():'Dag +'}</button>`;nameDrop.onclick=()=>openDayPartPicker(nameDrop,slot,'name');}const relativeId=state.relativeDays?.[slot];const term=relativeTerms.find(item=>item.id===relativeId);const relative=document.createElement('button');relative.type='button';relative.className=`week-picker-relative ${term?'filled':''}`;relative.innerHTML=term?`<span><span class="arrow">${term.arrow}</span>${term.label}</span>`:'Begrip +';relative.onclick=()=>openRelativePicker(relative,slot);head?.insertAdjacentElement('afterend',relative);const zone=column.querySelector('.activity-zone');if(zone){const activities=state.activities?.[slot]||[];const max=slot===3?4:7;zone.innerHTML=Array.from({length:max},(_,index)=>{const activity=activities[index];if(!activity)return `<button type="button" class="week-picker-slot" data-slot="${slot}" data-index="${index}" aria-label="Activiteit kiezen"><span class="slot-plus">+</span></button>`;return `<div class="week-picker-slot filled" data-slot="${slot}" data-index="${index}"><button type="button" class="week-picker-slot filled" data-slot="${slot}" data-index="${index}" aria-label="${activity.label} vervangen"><span class="activity-visual">${activityIconMarkup(activity.icon,activity.label)}</span><span class="slot-label">${activity.label}</span></button><button type="button" class="week-picker-remove" data-remove-slot="${slot}" data-remove-index="${index}" aria-label="Activiteit verwijderen">×</button></div>`;}).join('');zone.querySelectorAll('.week-picker-slot[data-slot][data-index]').forEach(button=>{if(button.closest('.week-picker-slot.filled')&&button.classList.contains('filled')&&button.tagName!=='BUTTON')return;button.onclick=event=>{if(event.target.closest('.week-picker-remove'))return;openActivityPicker(button,slot,+button.dataset.index)};});zone.querySelectorAll('.week-picker-remove').forEach(button=>button.onclick=event=>{event.preventDefault();event.stopPropagation();removeActivity(+button.dataset.removeSlot,+button.dataset.removeIndex)});}});mounting=false;}
